...
pub(crate) fn record_play_start(db: &Db, path: &str) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT INTO stats(track_id, play_count, last_played, skip_count)
         SELECT id, 0, ?2, 0 FROM tracks WHERE path = ?1
         ON CONFLICT(track_id) DO UPDATE SET last_played = ?2",
        params![path, now_ms()],
    )
...
pub(crate) fn record_play(db: &Db, path: &str) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT INTO stats(track_id, play_count, last_played, skip_count)
         SELECT id, 1, ?2, 0 FROM tracks WHERE path = ?1
...
pub(crate) fn record_skip(db: &Db, path: &str) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT INTO stats(track_id, play_count, last_played, skip_count)
         SELECT id, 0, 0, 1 FROM tracks WHERE path = ?1
...
pub fn db_stats_summary(db: State<Db>) -> Result<StatsSummary, String> {
    ...
    conn.query_row(
        "SELECT COALESCE(SUM(s.play_count), 0), COALESCE(SUM(s.play_count * t.duration_secs), 0)
         FROM stats s JOIN tracks t ON t.id = s.track_id",
        [],
        |r| {
            Ok(StatsSummary {
                total_plays: r.get(0)?,
...
pub fn db_rediscover(db: State<Db>, limit: i64) -> Result<Vec<MusicTrack>, String> {
    ...
    let max_id: i64 = conn
        .query_row(
            "SELECT COALESCE(MAX(t.id), 0)
             FROM tracks t JOIN favorites f ON f.track_id = t.id
    ...
    let forward = format!(
        "SELECT {TRACK_COLS_T} FROM tracks t JOIN favorites f ON f.track_id = t.id
         LEFT JOIN stats s ON s.track_id = t.id
...
pub fn db_insight_counts(db: State<Db>) -> Result<InsightCounts, String> {
    ...
    Ok(InsightCounts {
        recently_played: one(
            "SELECT COUNT(*) FROM stats s JOIN tracks t ON t.id = s.track_id WHERE s.last_played > 0",
            params![],
        )?,
        most_played: one(
            "SELECT COUNT(*) FROM stats s JOIN tracks t ON t.id = s.track_id WHERE s.play_count > 0",
            params![],
        )?,
        on_repeat: one(
            "SELECT COUNT(*) FROM stats s JOIN tracks t ON t.id = s.track_id
             WHERE s.play_count >= 2 AND s.last_played >= ?1",
            params![cutoff45],
        )?,
        ...
        rediscover: one(
            "SELECT COUNT(*) FROM favorites f JOIN tracks t ON t.id = f.track_id
             LEFT JOIN stats s ON s.track_id = f.track_id
             WHERE COALESCE(s.last_played, 0) = 0 OR s.last_played < ?1",
            params![cutoff60],
        )?,
    })
