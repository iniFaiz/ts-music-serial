...
#[tauri::command]
pub fn db_import_backup(app: AppHandle, db: State<Db>, src: String) -> Result<ImportBackupResult, String> {
    ...
    {
        let mut stmt = new_conn.prepare("SELECT path FROM roots").map_err(|e| e.to_string())?;
...
pub fn db_prune_and_get_missing(db: State<Db>) -> Result<Vec<MissingTrackInfo>, String> {
    ...
    {
        let mut stmt = conn.prepare("SELECT path, title, artist FROM tracks").map_err(|e| e.to_string())?;
    ...
    if !to_remove.is_empty() {
        let tx = conn.transaction().map_err(|e| e.to_string())?;
        for p in &to_remove {
            tx.execute("DELETE FROM tracks WHERE path = ?1", params![p]).map_err(|e| e.to_string())?;
            tx.execute("DELETE FROM stats WHERE path = ?1", params![p]).map_err(|e| e.to_string())?;
            tx.execute("DELETE FROM favorites WHERE path = ?1", params![p]).map_err(|e| e.to_string())?;
            tx.execute("DELETE FROM playlist_items WHERE path = ?1", params![p]).map_err(|e| e.to_string())?;
        }
        tx.commit().map_err(|e| e.to_string())?;
    }
