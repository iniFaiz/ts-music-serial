  ...
  <div class="w-full text-left text-sm 2xl:text-lg px-6 pb-12">
    ...
    <div
      class="grid gap-4 text-[var(--text-secondary)] text-xs 2xl:text-sm font-medium uppercase tracking-wide border-b border-[var(--border-color)] py-2 mb-2 sticky top-0 bg-[var(--app-bg)]/95 backdrop-blur-xl z-10 select-none
             grid-cols-[20px_3fr_2fr_2fr_120px]
             2xl:grid-cols-[40px_4fr_3fr_3fr_150px]"
    >
      ...
      >
        ...
        <div class="flex items-center justify-end gap-2 2xl:gap-3">
          <button
            ...
            :class="store.isFavorite(song.path)
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" :fill="store.isFavorite(song.path) ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
          </button>

          <span class="text-[12px] 2xl:text-[14px] text-[var(--text-secondary)] font-variant-numeric tabular-nums">{{ formatDuration(song.duration_secs) }}</span>
          ...
          <button
            ...
            title="More"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor" stroke="none"><circle cx="5" cy="12" r="1.6"></circle><circle cx="12" cy="12" r="1.6"></circle><circle cx="19" cy="12" r="1.6"></circle></svg>
          </button>
        </div>
      </div>
    ...
    <Teleport to="body">
      <div
        ...
        @contextmenu.prevent
      >
        <button @click="playNext" class="w-full text-left px-4 py-2 hover:bg-[#3a3a3a] transition-colors">Play next</button>
        <button @click="addToQueue" class="w-full text-left px-4 py-2 hover:bg-[#3a3a3a] transition-colors">Add to queue</button>

        <div class="border-t border-[#3a3a3a] my-1"></div>

        <button @click="toggleLike" class="w-full text-left px-4 py-2 hover:bg-[#3a3a3a] transition-colors">
          {{ menu.song && store.isFavorite(menu.song.path) ? 'Remove from Liked Songs' : 'Add to Liked Songs' }}
        </button>

        <div class="border-t border-[#3a3a3a] my-1"></div>

        <div class="px-4 py-1 text-[11px] uppercase tracking-wide text-gray-500">Add to playlist</div>
        ...
        <button @click="newPlaylistWithSong" class="w-full text-left px-4 py-2 text-[var(--accent-color)] hover:bg-[#3a3a3a] transition-colors">
          + New playlist
        </button>

        <template v-if="playlistId">
          <div class="border-t border-[#3a3a3a] my-1"></div>
          <button @click="removeFromThisPlaylist" class="w-full text-left px-4 py-2 hover:bg-[#3a3a3a] transition-colors">
