...
const initForm = () => {
  if (store.smartModal.mode === 'edit') {
    ...
    form.value = existing ? JSON.parse(JSON.stringify(existing)) : newSmartPlaylist();
  } else {
    ...
    <Transition name="modal">
      ...
      >
        ...
        >
          <!-- Header -->
          <div
            class="flex items-center justify-between px-6 py-4 border-b border-[#2c2c2e] shrink-0"
          >
            <div class="flex items-center gap-2.5">
              <div
                class="w-8 h-8 rounded-lg [background:color-mix(in_srgb,var(--accent-color)_15%,transparent)] flex items-center justify-center text-[var(--accent-color)]"
              >
                <svg
          ...
          <div class="px-6 py-5 overflow-y-auto scrollbar-thin space-y-5">
            ...
            <div class="space-y-2">
              ...
              <p
                v-if="usesGenre && !libraryHasGenre"
                class="text-[11px] text-amber-400/90 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2 mt-1"
              >
                None of your tracks have a genre tag yet. Re-scan / reindex your library (Settings)
                so genre rules can match.
              </p>
            </div>
